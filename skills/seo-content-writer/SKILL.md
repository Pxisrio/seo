...
6. **Structure Content and Write** — use a clean H1 > intro > H2/H3 > FAQ > conclusion flow.
   - Human editorial requirement: prose must sound like a real person wrote it, remove repeated or near-duplicate ideas, and make the article meaningfully distinct from similar existing content.
   - Product mention requirement: connect our product to the article topic only when the relationship is accurate and natural; do not misclassify our product as a category it does not belong to, do not use awkward disclaimers like "although our product is not...", and avoid repeating the same product use case across mentions.
7. **Apply On-Page Best Practices** — manage keyword placement, readability, snippets, and supporting visuals; one official website screenshot or product image suggestion for our product is enough per article when our product is discussed, and every suggested image must include SEO-friendly alt text tied to the image content and article context.
8. **Add Internal / External Links** — include context-relevant internal links and authoritative external links; vary external source domains, avoid competitor/product ad pages as evidence, and never add links or examples that feel unrelated to the article.
...
Match intent, front-load value, support claims with evidence, add relevant visuals and source links, and write for humans before optimizing for the SERP. The final draft should read like a real person wrote it: vary rhythm and sentence shape, merge or remove repeated ideas, avoid generic AI-style phrasing, add a distinct angle, and mention our product only where it naturally helps the reader understand the topic or next step.
